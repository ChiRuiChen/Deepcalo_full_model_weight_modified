...
template<class data_T, class res_T, typename CONFIG_T>
  void cnnshift_arr(data_T data[CONFIG_T::n_chan],
		    ap_shift_reg<data_T, CONFIG_T::in_width> line_buffer[MAX(CONFIG_T::filt_height - 1,1)][CONFIG_T::n_chan],
		    data_T output[(CONFIG_T::filt_height*CONFIG_T::filt_width)*(CONFIG_T::n_chan)]) { 
  
    #pragma HLS PIPELINE
    const static int rowsize = CONFIG_T::in_width;
	...
	data_T tmp       = line_buffer[i1-1][i0].shift(tmp1);
         ...
         typename CONFIG_T::bias_t   biases[CONFIG_T::n_filt]) 
{
	...
	std::cout<<"USE LARGE CONV2D"<<"std::endl";
	...
	//#pragma HLS PIPELINE II = 1
		...
		//Linebuffer
		nnet::cnnshift_arr<typename data_T::value_type, typename res_T::value_type,  CONFIG_T>(tmpdata, line_buffer, kernel_data);
		...
		if ( (sX - lShiftX) == 0 && (sY - lShiftY) == 0 && pY > lShiftY - 1 && pX > lShiftX - 1) {
			...
			if (CONFIG_T::strategy == nnet::latency) {
				dense_latency<typename data_T::value_type, typename res_T::value_type, typename CONFIG_T::mult_config>(kernel_data, layer_out, weights, biases);
			} else {
				dense_resource<typename data_T::value_type, typename res_T::value_type, typename CONFIG_T::mult_config>(kernel_data, layer_out, weights, biases);
			}
